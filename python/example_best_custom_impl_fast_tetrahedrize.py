...
def mmf_tetrahedron_convex_decomposition(fun_shape : manifold3d.Manifold) -> list[manifold3d.Manifold]:
    ...
    for shape in shapes:
        ...
        tet_vertices = np.vstack([unique_face_vertices, cube_corners])
        #tet_indices = tetrahedralize.tetrahedralize(tet_vertices, min_quality=0.0)#0.005)

        tet_indices = vocd.createTets(tet_vertices, minQuality=0.001)

        tet_indices  = np.array(tet_indices , dtype=np.uint32)
        tet_vertices = np.array(tet_vertices, dtype=np.float64)
        print(f"Tetrahedralized into {tet_indices.shape} tetrahedra from {tet_vertices.shape} input points")
        ...
        print("Voronoi Input:", points.shape, wts.shape, bounds)
        ...
        intersected_hulls1 = [fun_shape ^ hull for hull in hulls]

        print(f"Intersected with original shape to get {len(intersected_hulls1)} hulls")

        intersected_hulls = [hull for hull in intersected_hulls1 if not hull.is_empty()]
        for i in range(3):
            decomposed_hulls = []
